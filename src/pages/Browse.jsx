...
function Browse() {
  ...
  const handleGenreClick = async (genre) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`https://jiosavan-api-with-playlist.vercel.app/api/search/playlists?query=${genre.toLowerCase()}&limit=70`);
      // const response = await fetch(`https://saavn.dev/api/search/playlists?query=${genre.toLowerCase()}&limit=70`);
    ...
    return (
      <>
       ...
       hidden lg:block arrow-btn "
            ...
            <div
              ref={scrollRef}
              className=" grid lg:grid-rows-2 lg:grid-cols-none scroll-smooth grid-cols-2  lg:grid-flow-col-dense gap-[0.625rem] w-full px-[10rem] overflow-x-scroll scroll-hide">
              {playlists.map((genre) => (
